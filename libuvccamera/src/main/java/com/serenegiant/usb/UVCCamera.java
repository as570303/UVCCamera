...
public class UVCCamera {
    ...
    protected int mContrastMin = 0;
    protected int mContrastMax = 222;
    ...
    protected int mSaturationMin = 0;
...
//================================================================================
     ...
     * @param contrast [%]
     */
    public synchronized int setContrast(final int contrast) {
        if (mNativePtr != 0) {

            nativeUpdateContrastLimit(mNativePtr);
            final float range = Math.abs(mContrastMax - mContrastMin);
            Log.e("设置成功setContrast"  , "range ="+range );

            if (range > 0){
                return   nativeSetContrast(mNativePtr, (int) (contrast / 100.f * range) + mContrastMin);

            }else{
                return    nativeSetContrast(mNativePtr,  contrast);
            }
        }else {
            Log.e("设置成功"  , "setContrast ="+mNativePtr );
...
//================================================================================
     ...
     * @param saturation [%]
     */
    public synchronized int setSaturation(final int saturation) {
        if (mNativePtr != 0) {
            final float range = Math.abs(mSaturationMax - mSaturationMin);
            Log.e("设置成功"  , "range ="+range );
            if (range > 0){
                return      nativeSetSaturation(mNativePtr, (int) (saturation / 100.f * range) + mSaturationMin);

            }else{
              return    nativeSetSaturation(mNativePtr,   mSaturationMin);

            }
        }else{
            Log.e("设置成功"  , "mNativePtr ="+mNativePtr );
            return   nativeSetSaturation(mNativePtr,   mSaturationMin);
        }
